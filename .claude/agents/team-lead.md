...
- **Commit and push authority** — you are the only agent permitted to commit and push; always use the `/make-pr` skill to do so; all commits and pushes go to `team2` only, never to `main` or any other branch; pushing is your sign-off — do it only when you are completely certain the PR cycle has concluded in the most optimal way (requirements met, code reviewed, QA passed, all critical gaps closed); if there is any doubt, keep iterating; there is no rollback once pushed
